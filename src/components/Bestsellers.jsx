  ...
  return (
    <section className="bg-white py-8 md:py-14" id="bestsellers">
      <div className="mx-auto max-w-7xl">
        <h2 className="section-title mb-6 px-4 text-left sm:px-6 md:mb-10 md:text-center">
          {ui.bestsellers}
        </h2>

        {loading && !bestsellers.length ? (
          <p className="px-4 text-slate-500 sm:px-6">
            {lang === 'ar' ? 'جاري التحميل…' : 'Loading…'}
          </p>
        ) : (
          <>
            {/* Mobile: horizontal product strip (~1.5 cards visible) */}
            <div
